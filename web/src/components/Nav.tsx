...
export function Nav() {
  ...
  return (
    <nav className="border-b border-card-border bg-card/80 backdrop-blur-sm sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-sm text-muted">
          ...
          <a href="https://github.com/makoto-kusanagi/prisoners-arena-program" target="_blank" rel="noopener noreferrer"
             className="hover:text-foreground transition-colors mx-1" aria-label="GitHub">
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="sm:hidden border-t border-card-border bg-card px-4 py-3 space-y-1 text-sm">
          <a href="/docs" className="block text-muted hover:text-foreground transition-colors py-1.5">
            How It Works
          </a>
          <div className="pt-2 border-t border-card-border">
            <div className="text-xs font-semibold text-muted/60 uppercase tracking-wider pt-1 pb-2">Tools</div>
            {TOOLS.map(t => (
              <a key={t.href} href={t.href} className="block text-muted hover:text-foreground transition-colors py-1.5">
                {t.label}
              </a>
            ))}
          </div>
          <div className="pt-2 border-t border-card-border">
            <div className="text-xs font-semibold text-muted/60 uppercase tracking-wider pt-1 pb-2">Docs</div>
            {DOCS.map(t => (
              <a key={t.href} href={t.href} className="block text-muted hover:text-foreground transition-colors py-1.5">
                {t.label}
              </a>
          ...
          <div className="pt-2 border-t border-card-border flex">
            <NetworkSwitcher />
          </div>
        </div>
      )}
