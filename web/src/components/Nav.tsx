    ...
    <nav className="border-b border-[var(--card-border)] bg-white/80 backdrop-blur-sm sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-sm text-[var(--muted)]">
          <a href="/docs" className="hover:text-[var(--foreground)] transition-colors hidden sm:inline">
            How It Works
          </a>
          {/* Tools dropdown */}
      ...
      {menuOpen && (
        <div className="sm:hidden border-t border-[var(--card-border)] bg-white px-4 py-3 space-y-3 text-sm">
          <a href="/docs" className="block text-[var(--muted)] hover:text-[var(--foreground)] transition-colors">
            How It Works
          </a>
          {DOCS.map(t => (
